<!DOCTYPE html>
<html lang="en">

<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Order Confirmation - Garden of Eden Produce</title>

    <!-- SEO Meta Tags -->
    <meta name="description"
        content="Order confirmation page for Garden of Eden Produce. View your order details, products ordered, and total amount to pay.">
    <meta name="keywords" content="order confirmation, Garden of Eden Produce, organic produce, fresh vegetables, fruits">
    <meta name="author" content="Garden of Eden Produce">

    <!-- Open Graph / Facebook -->
    <meta property="og:type" content="website">
    <meta property="og:url" content="https://www.gardenofedenrwanda.com/order-confirmation">
    <meta property="og:title" content="Order Confirmation - Garden of Eden Produce">
    <meta property="og:description"
        content="View your order details from Garden of Eden Produce. Fresh, organic fruits and vegetables delivered to your doorstep.">
    <meta property="og:image" content="https://www.gardenofedenrwanda.com/images/logo.webp">

    <!-- Twitter -->
    <meta property="twitter:card" content="summary_large_image">
    <meta property="twitter:url" content="https://www.gardenofedenrwanda.com/order-confirmation">
    <meta property="twitter:title" content="Order Confirmation - Garden of Eden Produce">
    <meta property="twitter:description"
        content="View your order details from Garden of Eden Produce. Fresh, organic fruits and vegetables delivered to your doorstep.">
    <meta property="twitter:image" content="https://www.gardenofedenrwanda.com/images/logo.webp">

    <!-- Canonical URL -->
    <link rel="canonical" href="https://www.gardenofedenrwanda.com/order-confirmation">
    <link rel="stylesheet" href="https://rsms.me/inter/inter.css">
    <!-- Styles -->
    <?php echo app('Illuminate\Foundation\Vite')('resources/css/app.css'); ?>
    <?php echo app('Illuminate\Foundation\Vite')('resources/js/app.js'); ?>
    <style>
        @media print {
            .no-print {
                display: none;
            }
        }
    </style>
</head>

<body class="bg-gray-100 font-sans">
    <div class="container mx-auto px-4 pb-10 pt-10">
        <div class="max-w-4xl mx-auto  bg-white rounded-md shadow-sm py-10 px-10 pb-10 pt-10">
            <div class="my-8">
                <a href="<?php echo e(route('landing')); ?>" class="no-print inline-block mb-4">
                    <img src="<?php echo e(asset('images/logo.webp')); ?>" alt=""
                        class="max-h-24 rounded-full border-green-500">
                </a>

                <button
                    class="bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded float-right no-print text-sm"
                    onclick="window.print();">
                    <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 inline-block mr-1" viewBox="0 0 20 20"
                        fill="currentColor">
                        <path fill-rule="evenodd"
                            d="M5 4v3H4a2 2 0 00-2 2v3a2 2 0 002 2h1v2a2 2 0 002 2h6a2 2 0 002-2v-2h1a2 2 0 002-2V9a2 2 0 00-2-2h-1V4a2 2 0 00-2-2H7a2 2 0 00-2 2zm8 0H7v3h6V4zm0 8H7v4h6v-4z"
                            clip-rule="evenodd" />
                    </svg>
                    Print order
                </button>
                <div class="clear-both"></div>

                <h4 class="text-2xl font-bold my-4">Your order is received</h4>
                <div id="printOrder">
                    <table class="w-full mb-8">
                        <tbody>
                            <tr>
                                <td class="font-bold py-2">Order No</td>
                                <td class="py-2"> <?php echo e($order->order_no); ?></td>
                            </tr>
                            <tr>
                                <td class="font-bold py-2">Order date</td>
                                <td class="py-2"> <?php echo e(date('j M Y h:i a', strtotime($order->created_at))); ?></td>
                            </tr>
                            <tr>
                                <td class="font-bold py-2">Client</td>
                                <td class="py-2">
                                    <?php echo e($order->user === null ? $order->client_name : $order->user->name); ?>

                                </td>
                            </tr>
                            <tr>
                                <td class="font-bold py-2">Client phone</td>
                                <td class="py-2"> <?php echo e(\App\Helpers\Garden::formatPhoneUs($order->client_phone)); ?></td>
                            </tr>
                            <tr>
                                <td class="font-bold py-2">Email address</td>
                                <td class="py-2"> <?php echo e($order->email); ?></td>
                            </tr>
                            <tr>
                                <td class="font-bold py-2">Delivery address</td>
                                <td class="py-2"> <?php echo e($order->shipping_address); ?></td>
                            </tr>
                        </tbody>
                    </table>

                    <h4 class="text-xl font-bold mb-4">Products ordered</h4>
                    <table class="w-full border-collapse border border-gray-300 mb-8">
                        <thead>
                            <tr class="bg-gray-200">
                                <th class="border border-gray-300 p-2 text-left">Product</th>
                                <th class="border border-gray-300 p-2 text-left">Price</th>
                                <th class="border border-gray-300 p-2 text-left">Quantity</th>
                                <th class="border border-gray-300 p-2 text-left">Total</th>
                            </tr>
                        </thead>
                        <tbody>
                            <?php $__currentLoopData = $order->orderItems; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $orderItem): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                <tr>
                                    <td class="border border-gray-300 p-2"><?php echo e($orderItem->product->name); ?></td>
                                    <td class="border border-gray-300 p-2"><?php echo e($orderItem->formattedPrice()); ?></td>
                                    <td class="border border-gray-300 p-2"><?php echo e($orderItem->quantity); ?></td>
                                    <td class="border border-gray-300 p-2"><?php echo e($orderItem->formattedSubtotal()); ?>

                                    </td>
                                </tr>
                            <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                        </tbody>
                        <tfoot>
                            <tr>
                                <th colspan="3" class="border border-gray-300 p-2 text-right">Sub Total:</th>
                                <th class="border border-gray-300 p-2 text-left">
                                    <?php echo e(Cknow\Money\Money::RWF($order->orderItems()->sum('subtotal'))); ?>

                                </th>
                            </tr>
                            <tr>
                                <th colspan="3" class="border border-gray-300 p-2 text-right">Shipping:</th>
                                <th class="border border-gray-300 p-2 text-left">
                                    <?php if($order->delivery_method == 'delivery'): ?>
                                        <?php echo e($setting->formattedShippingFee()); ?>

                                    <?php else: ?>
                                        <?php echo e(0); ?>

                                    <?php endif; ?>

                                </th>
                            </tr>
                            <tr>
                                <th colspan="3" class="border border-gray-300 p-2 text-right">Total:</th>
                                <th class="border border-gray-300 p-2 text-left">
                                    <?php if($order->delivery_method == 'delivery'): ?>
                                        <?php echo e(Cknow\Money\Money::RWF($order->orderItems()->sum('subtotal') + $setting->shipping_fee)); ?>

                                    <?php else: ?>
                                        <?php echo e(Cknow\Money\Money::RWF($order->orderItems()->sum('subtotal'))); ?>

                                    <?php endif; ?>

                                </th>
                            </tr>
                            <tr>
                                <th colspan="3" class="border border-gray-300 p-2 text-right">
                                    <strong class="text-green-600">Total amount to Pay:</strong>
                                </th>
                                <th class="border border-gray-300 p-2 text-left">
                                    <strong class="text-green-600">
                                        <?php echo e(Cknow\Money\Money::RWF($order->orderItems()->sum('subtotal') + $setting->shipping_fee)); ?>

                                    </strong>
                                </th>
                            </tr>
                        </tfoot>
                    </table>
                    <div>
                        <p>
                            <strong class="font-bold">Note:</strong>
                            <span> <?php echo e($order->notes); ?></span>
                        </p>
                    </div>
                </div>
            </div>
        </div>
    </div>
</body>

</html>
<?php /**PATH C:\Users\minis\Herd\online-shopping\resources\views/orders/index.blade.php ENDPATH**/ ?>