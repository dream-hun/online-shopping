        ...
        @forelse($relatedProducts as $relatedProduct)
            <div class="bg-white rounded-lg shadow-lg p-4">
                ...
                <span wire:loading.remove
                      wire:target="addToCart({{ $relatedProduct->id }}), removeFromCart({{ $relatedProduct->id }})">
                    {{ $this->isInCart($relatedProduct->id) ? 'Remove from Cart' : 'Add to Cart' }}
                </span>
                    <span wire:loading wire:target="addToCart({{ $relatedProduct->id }})"
                    ...
                    <span wire:loading wire:target="removeFromCart({{ $relatedProduct->id }})"
                          class="flex items-center justify-center">
                    <svg class="animate-spin -ml-1 mr-3 h-5 w-5 text-white inline-block"
                         xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
                        <circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor"
