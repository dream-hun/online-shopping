    ...
    <div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        <div class="py-20 xl:grid xl:grid-cols-3 xl:gap-8">
            <div class="space-y-12">
                ...
                <ul role="list" class="mt-6 space-y-6">
                    @foreach ($categories as $category)
                        <li class="text-md">
                            <a href="/shop?selected_categories[0]={{ $category->id }}" wire:navigate
                                class="text-gray-300 hover:text-white">{{ $category->name }}</a>
                        </li>
                    @endforeach
                </ul>
            </div>
            <div>
                ...
                <ul role="list" class="mt-6 space-y-6">
                    <li class="text-md">
                        <a href="{{ route('landing') }}" wire:navigate class="text-gray-300 hover:text-white">Home</a>
                    </li>
                    <li class="text-md">
                        <a href="{{ route('shop') }}" wire:navigate class="text-gray-300 hover:text-white">Shop</a>
                    </li>
                    <li class="text-md">
                        <a href="{{ route('about-us') }}" wire:navigate class="text-gray-300 hover:text-white">About
                            us</a>
                    </li>
                    <li class="text-md">
            ...
            <div>
                ...
                <form class="mt-2 flex sm:max-w-md" action="{{ route('subscribe') }}" method="POST">
                    @csrf
        ...
        <div class="border-t border-gray-800 py-10">
            <p class="text-sm text-gray-400">Copyright © {{ date('Y') }} {{ config('app.name') }}.</p>
        </div>
    </div>
</footer>
